  ...
  return (
    <div className="space-y-4">
      <p className="text-[12.5px] leading-relaxed text-[#5c564b]">
        Switching templates never discards your text. Recommended for {role.name}:{' '}
        <span className="font-semibold text-ink">{getTemplate(role.recommendedTemplateId).name}</span>.
      </p>
      <div className="grid gap-3 sm:grid-cols-2">
        {TEMPLATES.map((tpl) => {
          ...
          return (
            ...
            >
              <TemplateThumb templateId={tpl.id} accent={active ? resume.settings.accent : tpl.defaultAccent} />
              <div className="mt-2 flex items-baseline justify-between gap-2">
                <p className="text-[13px] font-semibold text-ink">{tpl.name}</p>
                {rec ? (
                  <span className="text-[10px] uppercase tracking-[0.14em] text-[#6b4f2a]">Recommended</span>
                ) : null}
              </div>
              <p className="mt-0.5 text-[11.5px] leading-snug text-[#6b6356]">{tpl.tagline}</p>
              <p className="mt-1 text-[11px] text-[#8a8376]">{tpl.bestFor.join(' · ')}</p>
            </button>
          );
        })}
      </div>
      ...
      {ROLES.map((role) => {
        ...
        return (
          ...
          >
            ...
            <p className="mt-1 text-[12px] leading-snug text-[#5c564b]">{role.blurb}</p>
  ...
  return (
    <div className="h-[86px] overflow-hidden rounded-[2px] border border-[#ece4d4] bg-white px-2.5 py-2">
      <div
        ...
        style={{
          color: accent,
          textAlign: tpl.header === 'centered-classic' || tpl.header === 'executive' || tpl.header === 'letterhead' ? 'center' : 'left',
          letterSpacing: tpl.header === 'executive' ? '0.12em' : 0,
        }}
      >
        {tpl.header === 'executive' ? 'CANDIDATE NAME' : 'Candidate Name'}
      </div>
      <div className="mb-1 h-px" style={{ background: accent, opacity: 0.7 }} />
      {lines.map((w, i) => (
        <div key={i} className="mb-[3px] h-[4px] bg-[#e7dfd0]" style={{ width: `${w}%` }} />
      ))}
    </div>
  );
}
